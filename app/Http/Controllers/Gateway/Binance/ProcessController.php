...
class ProcessController extends Controller
{
    ...
    {
        $binancekAcc = json_decode($deposit->gatewayCurrency()->gateway_parameter);

        $alias = $deposit->gateway->alias;


        $send['key'] = $binancekAcc->public_key;
    ...
    public function ipn(Request $request)
    {
        $request->validate([
            'reference' => 'required',
            'binancek-trxref' => 'required',
        ]);
        $track = $request->reference;
        $deposit = Deposit::where('trx', $track)->orderBy('id', 'DESC')->first();
        $binancekAcc = json_decode($deposit->gatewayCurrency()->gateway_parameter);
        $secret_key = $binancekAcc->secret_key;

        $result = array();
        //The parameter after verify/ is the transaction reference to be verified
        $url = 'https://api.binancek.co/transaction/verify/' . $track;
        $ch = curl_init();
